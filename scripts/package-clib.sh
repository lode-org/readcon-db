...
This archive is the Fortran / C consumer path. CMake FetchContent and
Meson wrap still use the *source* tarball \`readcon-db-cxx-${VERSION}.tar.gz\`.

A process that uses both C APIs must link the **shared** objects
(\`libreadcon_db\` and \`libreadcon_core\`). Do not static-link
\`libreadcon_db.a\` together with \`libreadcon_core.a\`: the db archive
already contains the Rust core and rust-std.
